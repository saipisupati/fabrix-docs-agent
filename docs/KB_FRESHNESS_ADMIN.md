...
5. **Scrape + gate** runs check-live → scrape → audit, then ingest only if page hashes changed.
6. **Force rebuild** always runs ingest + `build_kb` + evals. Stop uvicorn first (Qdrant file lock).
