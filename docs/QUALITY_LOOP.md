...
Harness exit **0** only when all three hold (including readiness streak ≥ 2). Do not embed on docs.fabrix.ai until then.

## Step 0 — Prep

```bash
cd /path/to/fabrix-docs-agent
source venv/bin/activate
set -a && source .env && set +a
```

- Stop anything on `:8080` (local Qdrant file lock):
  ```bash
...
Harness exit 0 + `readiness_streak≥2` → ready to discuss embed/hosting (separate decision). Restart the API when finished:
