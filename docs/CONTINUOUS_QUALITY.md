...
- Post-demo KB freshness/admin: [KB_FRESHNESS_ADMIN.md](KB_FRESHNESS_ADMIN.md) (plan: [superpowers/plans/2026-08-13-kb-freshness-admin.md](superpowers/plans/2026-08-13-kb-freshness-admin.md))
...
Latest completed hostile pass: **cycle 7** (install/VM facet). **Cycle 8** in progress (platform update + VM / RDAF CLI path — demote `update_image_repository` / Studio sizing). **Cycle 9** added (day-2 ops / vague product asks from adhoc 12Q — Splunk→dashboard, schedule debug, tickets, cron, IP whitelist trap, SQL→CFXQL, SN→Slack agent, dashboard↔pipeline, Kafka timeout, pipeline test, on-prem K8s, concurrent dataset writes). **Cycle 10** added (invented named entities — AutoHeal / `@fake:made-up-bot` must abstain when ungrounded in excerpts). **Cycle 11** added (jailbreak / false-memory / authority-claim traps — `answer freely`, `you said earlier`, `I'm a Fabrix engineer`). **Cycle 12** added (full-script invention — no turnkey bash with invented tags/credentials). **Cycle 13** added (topic≠detail specificity — K8s version / Airflow comparison overclaim). **Cycle 14** added (ChatGPT bakeoff discovery — Splunk/Datadog/New Relic wiring, schedule debug, concurrent dataset / worker-limit honesty, `@snowv2` family fidelity, dashboard kickoff overclaim). **Cycle 15** added (thin-wiring depth — widen map/land/building-blocks detect, force `@family:op` + auth + asked sink via critique). **Cycle 16** added (customer ticket-title themes — Windows/VMware/OpenStack/CrowdStrike discovery, upgrade prereqs, schedule debug, SNOW enrichment, Prometheus/Kafka, dashboard access, Agentic-in-prod, fake PRB-hostname corruption; triage in `data/customer_ticket_topics.json`). **Cycle 17** added (leftover ticket debug/trap themes + Kafka inference-disclosure lock — pipeline failed/UAT, alerts→tickets, AIOPS not running, email miss, events no-data, contractual SLA / MFA / Cloud IP / Qdrant replication traps, live OOM/disk/CPU incident refuse via `_is_live_incident_ask` honesty gate; candidates in `data/cycle17_candidates.md`). **Cycle 18** added (SaaS hosting-infra OOS — Cloud IP/allowlist/CIDR/firewall; `salvage_partial_answer` respects gaps that declare the core ask uncovered). **Cycle 19** added (broaden gap phrasing `"does not specify"`; stepwise procedure grounding for rollback/audit how-tos — refuse invented bot+param workflows). **Cycle 20** added (broaden `_is_stepwise_procedure_ask` beyond verb allowlist — imperative frame + Fabrix object fallback for clone/pause/migrate-style asks). **Cycle 21** added (false-certainty on undocumented behavioral rules — who-wins / does-X-also-Y; broaden concurrent-write detector + cascade side-effect honesty). **Cycle 22** added (bot naming/uniqueness retrieve expansion toward SDK package rules — `sdk.md` routing for "same name in different pipelines"; Git-as-version-control framing → Studio Version History/Publish via `tool_equivalence_guardrail` + honesty, no invented Git export). **Cycle 23** added (embedding-retry timeout blowup — split connect/read timeout `(5, 20)`, multi-facet wall-clock deadline 15s, per-query embed timing logs, memoize `load_kb`/`load_embeddings`). **Cycle 24** added (tighten `_live_incident_has_documented_procedure` — scenario-specific OOM/disk-full/CPU-spike procedure phrase lists instead of bare keyword overlap, so OIA memory-config / ansible `run-playbook` pages no longer count as a documented OOM playbook). **Cycle 25** added (tenant security-action-request honesty gate for "enable MFA/SSO for our org" — refuse invented settings walkthroughs unless excerpts document that procedure; force Qdrant/vector-DB replication into `force_oos` private-infra traps). **Cycle 26** added (query expansion for incremental-load→bookmarks and general secrets/credentials→Vault vocabulary mismatches; ungrounded-bot stripper no longer leaves "a documented bot from the retrieved catalog" placeholder prose). **Cycle 27** added (service-pipeline comparison asks route to `svc_blueprints_cli` / `scheduled_pipelines` instead of AI Fabric event triggers; `pipeline_category_guardrail` stops inventing a parallel definition for undocumented "event-driven pipeline" category). **Cycle 28** added (Examples must be topic-relevant — drop Windows ACL dump leaks matched only on weak words like "access"; source titles cleaned of raw markdown headers/links/table debris with word-boundary truncation). **Cycle 29** added (Pipeline Builder UI asks — verify/syntax, clone bot, publish draft — bias retrieve toward `pipe_builder`, force-expand `beginners_guide/pipe_builder.md` when the KB intro card alone is thin, enrich that fact with Clone/Verify/Publish lines, and ground "clone a **bot**" separately from undocumented "clone a **pipeline**" so the agent stops inventing `rdac.py pipeline publish` or false-abstaining on documented Clone). **Cycle 30** added (bookmark / incremental-resume asks — widen detector beyond "incremental load", force-expand `data_control` + `cfxdm`, canonicalize `@dm:save_bookmark`→`@dm:save-bookmark`, and stop `@c:bookmark-loop` invention). **Cycle 31** added (edit-after-publish path — Published Pipelines report → edit saves to Draft → Publish again; stop inventing "go edit in Draft Pipelines"). **Cycle 32** added (dataset `temp:` vs invented `temp-` prefix fidelity — force-expand `pipe_builder` + canonicalize to the documented colon form). **Cycle 33** added (demo-week generalization — multi-intent NR→stream→dashboard, invented `@fabricops` bot, classic Pipeline Builder vs AI Fabric toolset, SN→PD slang, clone-entire-pipeline honesty, worker-count overclaim, live disk-full refuse, SaaS allowlist OOS, Jinja templates, formatting-template outbound, authority-claim trap, schedule cron debug). Next unused cycle id after 33 lands: **34**.
